const { Button, Card, Badge, Icon, Mark, SpecList } = window.McKaySWoodshopDesignSystem_ff08a8;

const WHAT_I_BUILD = [
  { icon: 'armchair', title: 'Furniture', copy: 'Tables, chairs and cabinets, built to order.' },
  { icon: 'library', title: 'Built-ins', copy: 'Shelving, closets and benches, scribed to the room.' },
  { icon: 'trees', title: 'Outdoor', copy: 'Decks, pergolas and planters in cedar or teak.' },
  { icon: 'hammer', title: 'Restoration', copy: 'Repair and refinishing for a piece worth saving.' },
];

const HOW_IT_WORKS = [
  { n: '01', title: 'Reach out', copy: 'Send a quote request with the idea, rough size and timeline.' },
  { n: '02', title: 'Get a quote', copy: 'A call to firm up details, then a price and lead time.' },
  { n: '03', title: 'Built to order', copy: 'Built in the shop, with photo updates along the way.' },
  { n: '04', title: 'Delivery or pickup', copy: 'Delivered locally, or ready for pickup near Barrie.' },
];
...
function Home({ go }) {
  return (
    <div>
      <section style={{ position: 'relative', background: 'var(--surface-inverse)', padding: 'var(--section-y) 40px', overflow: 'hidden' }}>
        {/* Full-bleed photo, undimmed — the panel below is a flat colour field, not a scrim. */}
        <Photo label="Shop interior" ratio="auto" style={{ position: 'absolute', inset: 0, borderRadius: 0, border: 0, background: 'var(--forest-600)' }} />
        <div style={{ position: 'relative', maxWidth: 'var(--container)', margin: '0 auto' }}>
          <div style={{ maxWidth: '38rem', padding: 'var(--space-8, 24px)', background: 'rgba(11,33,23,.72)', borderRadius: 'var(--radius-card, 5px)' }}>
        ...
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56, alignItems: 'start' }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 20, marginTop: 32, paddingTop: 32, borderTop: '1px solid var(--line-hairline)' }}>
              {[['06', 'Years in the shop'], ['240', 'Pieces built'], ['Solid wood', 'Materials, always']].map(([n, l]) => (
                <div key={l}>
      ...
      <section style={{ background: 'var(--surface-sunken)', padding: 'var(--section-y) 40px', borderTop: '1px solid var(--line-hairline)', borderBottom: '1px solid var(--line-hairline)' }}>
        <div style={{ maxWidth: 'var(--container)', margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56, alignItems: 'start' }}>
            ...
            <SpecList items={WHAT_I_BUILD.map((s) => ({
              label: <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><span style={{ color: 'var(--forest-700)', display: 'flex' }}><Icon name={s.icon} size={20} /></span>{s.title}</span>,
              value: s.copy,
            }))} />
          </div>

          <div style={{ marginTop: 'var(--section-y)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 56, alignItems: 'start' }}>
            <SectionHead eyebrow="How it works" title="From first message to finished piece." />
            <SpecList items={HOW_IT_WORKS.map((s) => ({
              label: <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}><span style={{ font: 'var(--type-spec)', color: 'var(--forest-700)' }}>{s.n}</span>{s.title}</span>,
              value: s.copy,
            }))} />
          </div>
        </div>
